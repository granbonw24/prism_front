...
const SCOPE_DESCENDANTS: Record<ScopeKey, ScopeKey[]> = {
  idRegion: ['idDepartement', 'idDrena', 'idIep'],
  idDepartement: ['idDrena', 'idIep'],
  idDrena: ['idIep'],
  idIep: [],
...
export class UtilisateursComponent implements OnInit, OnDestroy {
    ...
    try {
      await firstValueFrom(
        this.admin.createUser({
          ...
          nom: String(this.createForm.nom ?? '').trim() || null,
          prenoms: String(this.createForm.prenoms ?? '').trim() || null,
          dateNaissance: String(this.createForm.dateNaissance ?? '').trim() || null,
          lieuNaissance: String(this.createForm.lieuNaissance ?? '').trim() || null,
          datePriseService: String(this.createForm.datePriseService ?? '').trim() || null,
          dateDepartRetraite: String(this.createForm.dateDepartRetraite ?? '').trim() || null,
  ...
  openEdit(u: AppUserAdmin): void {
    ...
    this.editForm = {
      ...
      datePriseService: u.datePriseService ?? null,
      dateDepartRetraite: u.dateDepartRetraite ?? null,
      roleIds: [...(u.roleIds ?? [])],
    ...
    try {
      await firstValueFrom(
        this.admin.updateUser(id, {
          ...
          nom: String(this.editForm.nom ?? '').trim() || null,
          prenoms: String(this.editForm.prenoms ?? '').trim() || null,
          dateNaissance: String(this.editForm.dateNaissance ?? '').trim() || null,
          lieuNaissance: String(this.editForm.lieuNaissance ?? '').trim() || null,
          datePriseService: String(this.editForm.datePriseService ?? '').trim() || null,
          dateDepartRetraite: String(this.editForm.dateDepartRetraite ?? '').trim() || null,
